...
exports.channelNotificationEmail = async (userData) => {
  ...
  let msg = `You have been assign in Gamble Social channel by the Gamble Social Admin.
  To access your channel, log into your Gamble Social account,click on the
  Gamble Social icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `Gamble Social has approved your Connection application.`;
    } else {
      msg = `Gamble Social has unapproved your Connection application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
...
exports.notificationMailOnInvite = async (userData) => {
  ...
  const mailObj = {
    email: userData.email,
    subject: "Gamble Social notification",
    root: "../email-templates/notification.ejs",
    templateData: { name: name, msg: msg, url: redirectUrl },
  };
